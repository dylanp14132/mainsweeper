...
int main() {
    ...
    auto setText = [](sf::Text& text, const float x, const float y) {
        sf::FloatRect textRect = text.getLocalBounds();
        text.setOrigin(textRect.left + textRect.width/2.0f, textRect.top  + textRect.height/2.0f);
        text.setPosition(x, y);
    };

    std::function<void(Tile& tile)> revealTile;

    revealTile = [&revealTile](Tile& tile) {
        if (tile.isRevealed)
            return;
    ...
    while(welcome_window.isOpen()) {
        ...
        while(welcome_window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::TextEntered && typed_text_string.size() < 10) {
                if ( (event.text.unicode < 91 && event.text.unicode > 64) || (event.text.unicode > 96 && event.text.unicode < 123)  ) {
        ...
        for (int i = 0; i < rows1; i++) {
            for (int j = 0; j < cols1; j++) {
                ...
                for (int k = 0; k < 8; k++) {
                    ...
                    if (offsetI < 0 || offsetI >= rows1 || offsetJ < 0 || offsetJ >= cols1) {
    ...
    bool isAddingToLeaderboard = false;
    ...
    while (game_window.isOpen()) {
        ...
        while (game_window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left && !isLeaderboardOpen) {
                if (!isGameOver && !isPaused && debugButton.getGlobalBounds().contains(sf::Mouse::getPosition(game_window).x,sf::Mouse::getPosition(game_window).y)) {
                    isDebugModeOn = !isDebugModeOn;
                }
                if (!isGameOver && pausePlayButton.getGlobalBounds().contains(sf::Mouse::getPosition(game_window).x,sf::Mouse::getPosition(game_window).y)) {
                    isPaused = !isPaused;
                ...
                if (leaderboardButton.getGlobalBounds().contains(sf::Mouse::getPosition(game_window).x,sf::Mouse::getPosition(game_window).y)) {
                    isLeaderboardOpen = true;
                }
                if (faceButton.getGlobalBounds().contains(sf::Mouse::getPosition(game_window).x,sf::Mouse::getPosition(game_window).y)) {
                    board.clear();
                    a.clear();
                    count = 0;
                ...
                for (std::size_t i = 0; i < rows; i++) {
                    ...
                    for (std::size_t j = 0; j < cols; j++) {
                        if (!isGameOver && board[i][j].tileSprite.getGlobalBounds().contains(sf::Mouse::getPosition(game_window).x,sf::Mouse::getPosition(game_window).y)) {
                            if (!board[i][j].isFlagged && board[i][j].hasMine) {
                                ...
                                bool didBroChange = true;
                                for (std::size_t k = 0; k < board.size(); k++) {
                                    for (std::size_t l = 0; l < board[k].size(); l++) {
                                        if (!board[k][l].hasMine && !board[k][l].isRevealed) {
                                            didBroChange = false;
                                        }
                                    }
                                }

                                isGameWon = didBroChange;
                            }
                    ...
                    for (std::size_t j = 0; j < cols; j++) {
                        if (!isGameOver && board[i][j].tileSprite.getGlobalBounds().contains(sf::Mouse::getPosition(game_window).x,sf::Mouse::getPosition(game_window).y)) {
                            if (!board[i][j].isRevealed) {
                                ...
                                mineCount += (board[i][j].isFlagged) ? -1 : 1;
                            }
                            tileFound = true;
                            break;
                        }
        ...
        if (mineCount <= 999 && mineCount >= -99) {
            ...
            int firstDigit = abs(mineCount) % 10;
            int secondDigit = (abs(mineCount) / 10) % 10;
            int thirdDigit = (abs(mineCount) / 100) % 10;

            mineCounter[2].setTextureRect(sf::IntRect((firstDigit * 21), 0, 21, 32));
            mineCounter[1].setTextureRect(sf::IntRect((secondDigit * 21), 0, 21, 32));

            if (isNegative) {
                mineCounter[0].setTextureRect(sf::IntRect(210, 0, 21, 32));
            } else {
                mineCounter[0].setTextureRect(sf::IntRect((thirdDigit * 21), 0, 21, 32));
            }
        }

        finalTime = numToTime(time);

        if (finalTime <= 9959) {
            int firstDigit = finalTime % 10;
            int secondDigit = (finalTime / 10) % 10;
            int thirdDigit = (finalTime / 100) % 10;
            int fourthDigit = (finalTime / 1000) % 10;

            timer[3].setTextureRect(sf::IntRect((firstDigit * 21), 0, 21, 32));
            timer[2].setTextureRect(sf::IntRect((secondDigit * 21), 0, 21, 32));
            timer[1].setTextureRect(sf::IntRect((thirdDigit * 21), 0, 21, 32));
            timer[0].setTextureRect(sf::IntRect((fourthDigit * 21), 0, 21, 32));
        }
        ...
        if (isGameWon) {
            int index = 0;
            isAddingToLeaderboard = true;
            if (isAddingToLeaderboard) {
                finalTime = numToTime(finalTime);
                ...
                if (isOnLeaderboard) {
                    ...
                    for (std::size_t i = 1; i <= 5; i++) {
                        if (i == index) {
                            out << (finalTime / 1000) % 10 << (finalTime / 100) % 10 << ":" << (finalTime / 10) % 10 << finalTime % 10 << ", " << typed_text_string << "\n";
                        } else {
                            std::getline(ss, line);
                            out << line << '\n';
                        }
            ...
            width = cols * 16, height = rows * 16 + 50;
            ...
            while(leaderboard_window.isOpen()) {
                ...
                while (leaderboard_window.pollEvent(event2)) {
